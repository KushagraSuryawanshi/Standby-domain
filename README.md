...
Github actions performs the builds and pushes immutable Docker images to docker hub. Ec2 only pulls those images from docker hub and runs them as
containers. So the ec2 host itself only needs docker, docker compose, nginx, the production compose file, and production environment variables.
